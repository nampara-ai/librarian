...
`v0.1.0a68` is the latest public alpha. It includes local CLI workflows, a FastAPI service,
...
search/export with detailed result timestamps, privacy-hardened uploads, CLI/API document deletion
with side-effect-free symlink-guarded cleanup coverage, release automation, OSS governance files,
OCR correction quality warnings, repeated-tail output validation, and an external capability review
for Dolphin, olmOCR, and LongCut upgrade paths.
...
pip install "nampara_librarian-0.1.0a68-py3-none-any.whl[all]"
```
...
docker run --rm -p 8080:8080 \
  ...
  ghcr.io/nampara-ai/librarian:v0.1.0a68
```
