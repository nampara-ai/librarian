...
Version `1.1.5` is the stable production release. The default deployment is local or single-node: source documents and generated outputs stay in SQLite-backed local storage unless you configure an external model provider for cleaning, classification, or OCR correction.
...
python -m venv .venv
source .venv/bin/activate
pip install "nampara_librarian-1.1.5-py3-none-any.whl[all]"
```
