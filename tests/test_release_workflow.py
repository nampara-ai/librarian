...
def test_release_evidence_verifier_rejects_missing_eval_tags(tmp_path: Path) -> None:
    verifier = _load_release_evidence_module()
    eval_path = tmp_path / "eval.json"
    eval_path.write_text(
        """
...
def test_release_evidence_verifier_rejects_incomplete_corpus_page_statuses(
    tmp_path: Path,
) -> None:
    verifier = _load_release_evidence_module()
    corpus_path = tmp_path / "corpus.json"
          ...
          "cases": [
            {
              ...
              "processing_seconds": 0.2,
              "peak_memory_bytes": 1000,
              "page_status_counts": {"succeeded": 1, "failed": 1, "pending": 1},
              "page_source_counts": {"ocr": 1, "pending": 1},
              "page_warning_counts": {"ocr-page-failed": 1},
              "page_attempts": 2,
              "max_page_duration_ms": 10.0,
              "ocr_pages": 1,
              "corrected_pages": 0,
...
def test_release_evidence_verifier_rejects_pdf_and_ocr_tag_spoofing(
    tmp_path: Path,
) -> None:
    verifier = _load_release_evidence_module()
    corpus_path = tmp_path / "corpus.json"
    corpus_path.write_text(
        """
        {
          ...
          "cases": [
            {
              "name": "spoofed ocr case",
              "passed": true,
              "tags": ["pdf", "ocr", "mixed-embedded-scanned"],
              "source_path": "corpus/one.md",
              "output_path": "converted/one.md",
              "input_bytes": 1000,
              "output_chars": 800,
