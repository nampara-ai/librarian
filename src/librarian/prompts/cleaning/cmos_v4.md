...
5. Implicit structure: when the source is plain text whose structure is only implied by convention,
   render that structure as Markdown headings. A title line at the very start of the document
   (optionally with a byline such as "by ...") becomes a `#` heading. A standalone chapter or
   section line — "Chapter 3", "Introduction", "Prologue", "Part Two", "Epilogue",
   "Appendix B" — becomes a `##` heading; when such a line is immediately followed by its title on
   the next line, join them into one heading (e.g. `## Chapter 3: The Deal`). A title/byline/
   copyright block that repeats before each chapter is a running header: keep it as the `#` heading
   at its first occurrence and omit the repetitions. Only promote lines the source visually sets
   apart as headings; never invent a heading for an ordinary sentence, and leave documents that
   already use Markdown headings unchanged.
6. Fidelity: do not summarize, paraphrase away detail, delete substantive content, invent facts,
...
7. Verbatim data: reproduce numbers, currency amounts, percentages, units of measure, dates, times,
...
8. Continuity: when the user input begins with a context note, use it only to maintain continuity.
