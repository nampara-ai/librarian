...
docker run --rm -p 8080:8080 \
  ...
  ghcr.io/nampara-ai/librarian:v1.9.0
```
...
LIBRARIAN_LLM_MAX_CONCURRENCY=8
LIBRARIAN_LLM_TIMEOUT_SECONDS=120
LIBRARIAN_LLM_MAX_RETRIES=5
LIBRARIAN_LLM_RETRY_BASE_DELAY_SECONDS=0.5
LIBRARIAN_LLM_RETRY_MAX_DELAY_SECONDS=10
LIBRARIAN_LLM_MAX_OUTPUT_TOKENS=16384
...
LIBRARIAN_CLEANING_PROMPT_VERSION=cmos_v5  # cmos_v1 through cmos_v5
LIBRARIAN_CLASSIFICATION_PROMPT_VERSION=dewey_v5  # dewey_v1 through dewey_v5
LIBRARIAN_CLASSIFICATION_MAX_OUTPUT_TOKENS=2048
LIBRARIAN_FINAL_REFINEMENT_ENABLED=true
LIBRARIAN_FINAL_REFINEMENT_MAX_ACTIONS=32
LIBRARIAN_FINAL_REFINEMENT_MAX_PAGE_CHARS=12000
LIBRARIAN_IMPORT_CONCURRENCY=2
...
The final refinement pass makes at most one model call for each localized issue it can review,
up to `LIBRARIAN_FINAL_REFINEMENT_MAX_ACTIONS`. It can reject or defer an edit if the source does
not support it, the provider fails, or a fidelity check fails. Review the quality report before
relying on a converted document; see [Operations](OPERATIONS.md#run-quality-and-provider-failures).
